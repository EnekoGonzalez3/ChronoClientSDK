...
int main() {
    ...
    ConnectResult connect_result = client.connect(client_account, client_host_ip, client_pid);

    if (connect_result.status == 0) {
        std::cout << "Connected successfully! Client ID: " << connect_result.client_id << std::endl;
    } else {
        std::cout << "Connection failed with code: " << connect_result.status << std::endl;
        return 1;
    }
    ...
    int create_rc = client.createChronicle(connect_result.client_id, "TestChronicle", attrs, flags);
    if (create_rc == 0) {
    ...
    // Example: Disconnect
    int disconnect_rc = client.disconnect(connect_result.client_id);
    if (disconnect_rc == 0) {
